<?php
  ...
  if(metadata('item', array('Dublin Core', 'Title'))) {
  	...
  	foreach ($titres as $i => $titre) {
  		echo "<span id='doc-title' style='font-size:$title_size; clear:both;display:block;'>$titre</span>";
  	}
  } else {
  	echo "<span id='doc-title' style='font-size:$title_size'>$title</span>";
  }
...
//   strpos($auteurs, ';') ? $label = $this->controller->t('Auteurs') : $label = $this->controller->t('Auteur');
  $label = $this->controller->t($author_name);
?>
<br /><span class="dclabel" style="font-size:<?php echo $author_size ?>;"><?php echo $label; ?></span><!--  (DC.Creator) --> : <span class="dcvalue" style="font-size:<?php echo $author_size ?>;"><?php echo $auteurs;?></span><br />
<?php endif; ?>
